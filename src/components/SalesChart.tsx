...
const SalesChart = ({ data, productName, brandName }: SalesChartProps) => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const minSales = d3.min(chartData, (d) => d.sales) ?? 0;
      const maxSales = d3.max(chartData, (d) => d.sales) ?? 0;
      const padding  = (maxSales - minSales) * 0.1;

      const yScale = d3.scaleLinear()
        .domain([Math.max(0, minSales - padding), maxSales + padding])
        .range([height, 0])
        .nice();
      ...
      // Area fill
      const area = d3.area<{ month: string; sales: number }>()
      ...
      svg.selectAll(".dot")
        ...
        .on("mouseenter", (event, d) => {
          ...
          tooltip
            .style("opacity", "1")
            .html(`<strong>${d.month}</strong><br/>${d3.format(",")(d.sales)} units`);
        })
        .on("mousemove", (event) => {
          tooltip
      ...
      // Min / Max labels
      const minPoint = chartData.reduce((a, b) => a.sales < b.sales ? a : b);
      const maxPoint = chartData.reduce((a, b) => a.sales > b.sales ? a : b);
      ...
      [
        ...
        svg.append("text")
          ...
          .attr("y",           yScale(point.sales) + offset)
          .attr("text-anchor", "middle")
          .attr("font-size",   "11px")
          .attr("fill",        color)
          .attr("font-weight", "600")
          .text(`${label}: ${d3.format(",")(point.sales)}`);
      });
    };
    ...
    return () => {
      ...
      d3.select("body").selectAll(".sales-tooltip").remove();
    };
