...
const SellsTrackerPage = () => {
  ...
  const [salesData, setSalesData] = useState<Record<string, number> | null>(null);
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const salesRes = await fetch(`${API}/sales-history?product_id=${productData.id}`);
      if (!salesRes.ok) { setError("No sales history found for this product."); return; }
...
export default SellsTrackerPage;